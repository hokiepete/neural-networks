...
def preprocess_cat_vs_dog_largeRam(folder = './data/dogs-vs-cats/train/',targ_height = 200,targ_width = 200):
    ...
    for file in listdir(folder):
        ...
        if file.startswith('cat'):
            output = 1.0
        #load image
        photo = load_img(folder+file, target_size=(targ_height,targ_width))
        #Convert to numpy
...
def define_model(targ_height,targ_width):
    model = Sequential()
    model.add(Conv2D(32,(3,3),activation='relu',kernel_initializer='he_uniform',padding='same',input_shape=(targ_height,targ_width,3)))
